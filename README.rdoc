...
== Default scope at the <tt>apply_scopes</tt> level

You can also set default scopes at the <tt>apply_scopes</tt> level, overriding any scopes with the <tt>:default</tt> option and called when no scope is explicitly given in the params.
...
* Changed <tt>has_scope</tt>'s <tt>:default</tt> option behavior, to has a real "default" behavior and not an "always" behavior.

* Added <tt>has_scope</tt>'s <tt>:always</tt> option allowing the same behavior as the former <tt>has_scope</tt>'s <tt>:default</tt> option.
